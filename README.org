...
Plugin for [[https://github.com/anishathalye/dotbot][dotbot]] that adds =brew=, =cask=, =tap=, and =brewfile= directives. It
allows installation of packages using Homebrew on macOS and Linux. The =cask=
directive is only supported on macOS. If =brew= is not installed, it will be
